...
namespace SelfContainedSolutionTest.Plugins.CustomAPIs
{
    ...
    public class Example : PluginBase
    {
        ...
        protected override void ExecuteDataversePlugin(ILocalPluginContext localContext)
        {
            ...
            {
                tracingService?.Trace("An error occurred executing Plugin SelfContainedSolutionExample.Plugins.Example : {0}", ex.ToString());
                throw new InvalidPluginExecutionException("An error occurred executing Plugin SelfContainedSolutionExample.Plugins.Example .", ex);
            }
        }
    }
}